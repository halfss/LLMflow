...
function subscribeToSSE() {
  if ('EventSource' in window) {
    const source = new EventSource(
      'http://records.ashfire.cn:7862/stream',
    )
    source.onmessage = function (e) {
      console.log(e)
      // // 监听信息的传输
      const message = prompt.value
      const response = JSON.parse(e.data).response
      const options: Chat.ConversationRequest = {}
      text.value = text.value + response
...
<template>
  <div class="flex flex-col w-full h-full bg-green-50">
    ...
    <main class="flex-1 overflow-hidden">
      <div class="main-content-header w-full max-w-screen-xl m-auto dark:bg-[#101014]" style="margin-top:7px" />
      <div id="scrollRef" ref="scrollRef" class="height-calc  overflow-y-auto  w-full max-w-screen-xl m-auto dark:bg-[#101014] scrollbar">
        <div
          id="image-wrapper"
    ...
    <footer :class="footerClass" style="margin-bottom: 7px;" class="footer-style w-full max-w-screen-xl m-auto ">
